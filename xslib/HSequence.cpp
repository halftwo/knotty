...
#ifdef XSLIB_RCSID
static const char rcsid[] = "$Id: HSequence.cpp,v 1.2 2012/09/20 03:33:09 jiagui Exp $";
#endif

HSequence::HSequence(const std::vector<xstr_t>& buckets, uint32_t keymask)
{
	_cache.ptr = NULL;
...
HSequence::HSequence(const std::vector<xstr_t>& buckets, const std::vector<int>& weights, uint32_t keymask)
{
	_cache.ptr = NULL;
	_mask = keymask ? round_up_power_two(keymask) - 1 : UINT32_MAX;
