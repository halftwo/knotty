/* $Id: HSequence.h,v 1.2 2012/09/20 03:21:47 jiagui Exp $ */
#ifndef HSequence_h_
#define HSequence_h_
...
{
public:
	HSequence(const std::vector<xstr_t>& buckets, uint32_t keymask);
	HSequence(const std::vector<std::string>& buckets, uint32_t keymask);

	HSequence(const std::vector<xstr_t>& buckets, const std::vector<int>& weights, uint32_t keymask);
