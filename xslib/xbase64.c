...
#ifdef XSLIB_RCSID
static const char rcsid[] = "$Id: xbase64.c 379 2016-10-12 05:09:07Z gremlin $";
#endif

const xbase64_t std_xbase64 = {
...
ssize_t xbase64_encode(const xbase64_t *b64, char *out, const void *in, size_t len, int flag)
{
	...
	int c0, c1, c2;
	int remain;
	bool auto_newline = (flag & XBASE64_AUTO_NEWLINE);
...
ssize_t xbase64_decode(const xbase64_t *b64, void *out, const char *in, size_t len, int flag)
{
	...
	for (r = 0, n = 0; s < end && (c = *s++) != 0; )
	{
		...
		if (x >= 64)
		{
			if (x == 64)
				break;
			else if (flag & XBASE64_IGNORE_NON_ALPHABET)
			{
				while (s < end && (c = *s++) != 0 && (x = detab[c]) > 64)
					continue;
				if (x >= 64)
					break;
			}
			else if (flag & XBASE64_IGNORE_SPACE)
			{
				while (s < end && (c = *s++) != 0 && ((x = detab[c]) > 64) && isspace(c))
					continue;
				if (x >= 64)
					break;
			}
...
int main()
{
	...
	assert(xbase64_init(&b64, std_xbase64.alphabet) >= 0);
	len = xbase64_encode(&b64, buf, abc, abc_len, XBASE64_AUTO_NEWLINE);
	printf("%d:\n%s\n", len, buf);

	memset(abc, 0, abc_len);
	len = xbase64_decode(&b64, abc, buf, -1, XBASE64_IGNORE_SPACE);
	printf("%d:\n%s\n", len, abc);
