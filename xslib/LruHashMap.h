/* $Id: LruHashMap.h,v 1.2 2012/09/20 03:21:47 jiagui Exp $ */
...
public:
	...
	LruHashMap(size_t slot_num, size_t max_num = 0);
	~LruHashMap();

	size_t count() const 		{ return _total; }
	size_t max_number() const 	{ return _max_num; }
	size_t slot_number() const 	{ return _slot_num; }
...
LruHashMap<Key, Data, HashFun>::LruHashMap(size_t slot_num, size_t max_num)
{
	if (max_num == 0)
	{
		_max_num = slot_num;
	...
	_total = 0;
	_slot_num = round_up_power_two(slot_num);
	if (_slot_num < MIN_SLOT_NUMBER)
		_slot_num = MIN_SLOT_NUMBER;
	_mask = _slot_num - 1;
	_tab = (Node **)calloc(_slot_num, sizeof(Node*));
...
LruHashMap<Key, Data, HashFun>::insert(const Key& key, const Data& val)
{
	...
	if (_total >= _max_num || (node = new(&_pool) Node(key, val)) == NULL)
	{
