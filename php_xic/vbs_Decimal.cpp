...
static void delete_Decimal(void *p TSRMLS_DC)
{
	zend_objects_free_object_storage(static_cast<zend_object*>(p) TSRMLS_CC);
}

static zend_object_value new_Decimal(zend_class_entry *ce TSRMLS_DC)
{
	...
	retval.handle = zend_objects_store_put(obj, NULL, delete_Decimal, NULL TSRMLS_CC);
...
{
	zval *zv = zend_read_property(classEntry_Decimal, getThis(), "s", sizeof("s") - 1, 0 TSRMLS_CC);
